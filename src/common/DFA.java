...
public class DFA extends FiniteAutomata {
	Function<Pair<State, Symbol>, State> func;

	public DFA(ComparableSet<State> Q, ComparableSet<Symbol> S, Function<Pair<State, Symbol>, State> func, State q0,
			ComparableSet<State> F) {
		super(Q, S, q0, F);
		this.func = func;
	}
	...
	public DFA minimalize() {
		...
		for (Pair<State, Symbol> entry : func.getMapping().keySet()) {
			Pair<State, Symbol> key = new Pair<State, Symbol>(intTomDFA.get(dfaToInt.get(entry.getFirst())),
					entry.getSecond());
